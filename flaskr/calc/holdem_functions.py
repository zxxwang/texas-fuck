...
def results(hole_cards, winner_list, result_histograms):
    retsult = {}
    float_iterations = float(sum(winner_list))
    for index, hole_card in enumerate(hole_cards):
        winning_percentage = float(winner_list[index + 1]) / float_iterations
        if hole_card == (None, None):
            ...
            retsult[str(hole_card) + " winning rate"] = str(winning_percentage)[0:5]
    retsult["Tie rate"] = str(float(winner_list[0]) / float_iterations)[0:5]
    for player_index, histogram in enumerate(result_histograms):
        player = {}
        for index, elem in enumerate(histogram):
            player[hand_rankings[index]] = str(float(elem) / float_iterations)[0:5]
        retsult["player" + str(player_index+1)] = player
    return retsult
