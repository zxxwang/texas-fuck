import time
import flaskr.calc.holdem_functions as holdem_functions
import flaskr.calc.holdem_argparser as holdem_argparser


def main(board=[], hole_cards=[], exact=False):
    num = 100000
    file_name = None
    hole_cards, board = holdem_argparser.check_input(board, hole_cards)
    # hole_cards, num, exact, board, file_name = holdem_argparser.parse_args()
    result = run(hole_cards, num, exact, board, file_name, True)
    print(result)
    return result
...
def run_simulation(hole_cards, num, exact, given_board, deck, verbose):
    ...
    #                                    result_histograms)
    return holdem_functions.results(hole_cards, winner_list,
                                    result_histograms)


if __name__ == '__main__':
    start = time.time()
    main(hole_cards=['Ad', 'Kd'], board=['6d', '9d', '2h'])
